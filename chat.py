...
def get_response(msg, threshold=0.5):
    ...
    print(f"Predicted tag: {tag}, Confidence: {prob.item()}")  # Debug line

    # Check if predicted intent confidence exceeds threshold
    if prob.item() > threshold:
        for intent in intents['intents']:
            if tag == intent["tag"]:
                return random.choice(intent['responses'])

    # If confidence is below threshold or no matching intent found, use Palm API
